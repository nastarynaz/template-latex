...
% {\FAPET} article template
% {\FAPETConference} conference template

\newcommand\fapettex{FAPET~\LaTeX}
...
\pages{30-36}
\thisyear{2025}
\doi{\href{https://fapet.ugm.ac.id/}{https://fapet.ugm.ac.id/}}
...
\title{\fapettex ~ template for article preparation  v4.1}
...
\section{PREAMBLE}
    ...
    \bigskip
    \CS{documentclass}\verb+{fapet-rho}+
    
    \bigskip
    or
    
    \bigskip
    \CS{documentclass}[\textbf{optionlist}]\verb+{fapet-rho}+
    
    \bigskip
    \noindent which configures the document to use the fapet class, using the default \texttt{manuscript} option, which is designed for use by authors who submit articles to the main FAPET journal. The following commands can be used after the \verb|documentclass| command, but before \verb|\begin{document}|.
...
\subsection{Sectioning commands and cross references}
    ...
    These will be automatically typed in the FAPET style. Cross-referencing is made easier by the use of the \CS{label} \verb+{LABEL}+ command immediately after each sectioning command, where the LABEL text is a mnemonic string. Elsewhere in the document, the section can then be referred to as \S \CS{ref} \verb+{LABEL}+. The \CS{label} command can also be used with equations, figures, and tables (see below). 
...
\end{verbatim}
    ...
    The new \fapettex\ macro allows the authors to use a colored box to highlight a concept or equation, as shown in this example. The label and reference points of the section are included. Example: See the Concept box in Section~\ref{box:box1}.  
...
\subsection{Landscape table}
 ...
 FAPET uses the \LaTeX package \texttt{tabular}, which is adequate for most applications in this context. A good tutorial for tables, with direct applications of the \texttt{tabular} can be found in \url{https://www.overleaf.com/learn/latex/Tables}. 

 We do not recommend publishing very long tables in your paper. Large data collections may be more useful to readers in a machine-readable format (MRT). In the manuscript, a table with fewer rows may serve as a guide for the content of the MRT document. Please contact our editors to allocate direct link access to MRT tables in the FAPET web page for the published version of your manuscript. 
...
\section{REFERENCE STYLE}
    ...
    Our default formatting for references uses the journal-naming system from the AASTex style and the Astrophysics Data System (ADS). FAPET follows the ADS bibliographic codes for both refereed \url{https://adsabs.harvard.edu/abs_doc/refereed.html} and non-refereed \url{https://adsabs.harvard.edu/abs_doc/non_refereed.html} publications.
    ...
    bib file. The {\bf fapet.bib file} is an example of a bib file that serves as a guide.
%----------------------------------------------------------
